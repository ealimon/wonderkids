...
export default function MathSubtraction({
  ...
  const handleGenerateWorksheet = (diff = wsDifficulty, count = wsProblemCount, theme = wsTheme) => {
    ...
    for (let i = 1; i <= count; i++) {
      ...
      if (diff === 'easy') {
        num1 = Math.floor(Math.random() * 7) + 4; // 4 to 10
        num2 = Math.floor(Math.random() * (num1 - 1)) + 1; // result >= 1
      } else if (diff === 'medium') {
        num1 = Math.floor(Math.random() * 11) + 10; // 10 to 20
        num2 = Math.floor(Math.random() * (num1 - 2)) + 2; // result >= 2
      } else {
        // Advanced
        num1 = Math.floor(Math.random() * 70) + 30; // 30 to 100
        num2 = Math.floor(Math.random() * (num1 - 10)) + 5; // result >= 5
      }
  ...
  // Re-generate problems when customization option changes
  useEffect(() => {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto flex flex-col items-center select-none text-black" id="math-subtraction-game">
            ...
            {rounds.length > 0 && activeProblem ? (
              <div className="w-full bg-teal-100 rounded-[36px] border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] p-6 sm:p-8 flex flex-col justify-between min-h-[380px] relative overflow-hidden">
                ...
                <div className="text-center mb-6">
                  <div className="inline-block bg-teal-300 border-3 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] p-3 rounded-2xl mb-3 text-2xl">➖</div>
                  <h2 className="text-2xl sm:text-4xl font-black uppercase tracking-tight">SUBTRACTION SAFARI</h2>
                  <div className="bg-teal-200/80 border-2 border-black px-4 py-2 rounded-2xl inline-block mt-2 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                ...
                <div className="flex flex-col md:flex-row items-center justify-center gap-6 my-6">
                  ...
                  <div className="flex flex-col items-center">
                    <div className="w-32 h-32 sm:w-36 sm:h-36 bg-white border-4 border-black rounded-[32px] shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] flex items-center justify-center relative">
                      <motion.span
                        ...
                        className="text-6xl sm:text-7xl font-black text-teal-600 font-mono select-none"
                      >
                        {activeProblem.total}
                      </motion.span>
                    </div>
                  ...
                  {/* Minus sign */}
                  <div className="bg-teal-300 border-3 border-black rounded-full p-3 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">
                  ...
                  <div className="flex flex-col items-center">
                    <div className="w-32 h-32 sm:w-36 sm:h-36 bg-white border-4 border-black rounded-[32px] shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] flex items-center justify-center relative">
                      <motion.span
                        animate={roundComplete ? { scale: [1, 1.25, 1], rotate: [0, 10, -10, 0] } : {}}
                        className="text-6xl sm:text-7xl font-black text-red-500 font-mono select-none"
                      >
                  ...
                  <div
                    ...
                    className="w-32 h-32 sm:w-36 sm:h-36 bg-teal-300 border-4 border-black rounded-[32px] shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] flex items-center justify-center relative"
                  >
                    <AnimatePresence mode="wait">
                      {roundComplete ? (
                        ...
                        >
                          ...
                          <span className="text-xs sm:text-sm font-black uppercase text-teal-950 mt-1">
                            CORRECT!
                          </span>
                        </motion.div>
                      ) : (
                    ...
                    {roundComplete ? (
                      ...
                      >
                        ...
                        {!gameComplete ? (
                          ...
                          <div className="flex flex-col items-center">
                            <span className="text-2xl sm:text-3xl text-teal-950 font-black uppercase tracking-widest font-sans">SUBTRACTION MASTER! 👑🦁</span>
                            <span className="text-lg font-extrabold text-teal-900 mt-1">Incredible subtracting skills! You solved all subtraction levels!</span>
                          </div>
                        )}
                      </motion.div>
                    ) : (
                      <div className="flex flex-col items-center w-full" key="options-picker">
                        <span className="text-base sm:text-lg font-black uppercase tracking-wider text-teal-950 mb-4 font-sans flex items-center gap-2">
      ...
      <div className={`w-full flex-col gap-8 text-black ${activeTab === 'worksheet' ? 'flex' : 'hidden print:flex'}`}>
            ...
            {/* LIVE PAPER PREVIEW CONTAINER (Styled like real standard paper) */}
            <div className="w-full flex justify-center print:w-full print:m-0 print:p-0">
              ...
              >
                ...
                <div className="flex flex-col sm:flex-row justify-between items-center gap-4 border-b-4 border-black pb-6 mb-8 print:flex-row print:justify-between print:pb-2 print:mb-3">
                  ...
                  {/* Worksheet meta details like name and date */}
                ...
                <div className={`grid ${wsProblemCount === 12 ? 'grid-cols-2 sm:grid-cols-3 print:grid-cols-3' : 'grid-cols-2 sm:grid-cols-4 print:grid-cols-4'} gap-6 print:gap-x-4 print:gap-y-3`}>
                  {wsProblems.map((prob, idx) => (
                    ...
                    >
                      ...
                      {/* Dotted target container for solution */}
                ...
                {/* ANSWER KEY PAGE (Forced to next page on Print, displayed with separation on-screen) */}
                {wsShowAnswers && (
                  <div className="print-break-before mt-16 pt-12 border-t-4 border-dashed border-purple-300 relative print:border-none print:mt-0 print:pt-0">
                    ...
                    {/* Visual spacer only shown on web preview */}
                    ...
                    <div className={`grid ${wsProblemCount === 12 ? 'grid-cols-2 sm:grid-cols-3 print:grid-cols-3' : 'grid-cols-2 sm:grid-cols-4 print:grid-cols-4'} gap-6 print:gap-x-4 print:gap-y-3`}>
                      {wsProblems.map((prob, idx) => (
                        ...
                        >
                          ...
                          {/* Computed Answer formatted in colored bold text resembling marker correction */}

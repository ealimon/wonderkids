...
export default function WordPhonics({
  ...
  const handlePrint = async () => {
    audioManager.playPop();
    const paperElement = document.getElementById('word-phonics-worksheet-paper');
    setIsExporting(true);
    await exportOrPrintElement({
      element: paperElement,
      filename: `Storybook_Phonics_Worksheet_${wsMascotTheme}`,
      title: `Word Phonics Worksheet - ${getMascotDetails().title}`,
    ...
    <div className="w-full max-w-5xl mx-auto flex flex-col items-center select-none text-black" id="phonics-game-container">
      ...
      <div className={`w-full flex-col gap-8 text-black ${activeTab === 'worksheet' ? 'flex' : 'hidden print:flex'}`}>
            ...
            <div className="w-full flex justify-center print:w-full print:m-0 print:p-0">
              ...
              `}} />
              <div
                id="word-phonics-worksheet-paper"

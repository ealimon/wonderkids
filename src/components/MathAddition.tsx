...
export default function MathAddition({
  ...
  const handleDragEnd = (event: any, info: any, optionVal: number) => {
    ...
    const targetEl = document.getElementById('addition-answer-target');
    let hit = false;
    ...
    if (!hit) {
      ...
      if (el && el.closest('#addition-answer-target')) {
        hit = true;
      }
    }
  ...
  const handleGenerateWorksheet = (diff = wsDifficulty, count = wsProblemCount, theme = wsTheme) => {
    ...
    for (let i = 1; i <= count; i++) {
      ...
      if (diff === 'easy') {
        ...
        num2 = Math.floor(Math.random() * Math.min(10, maxNum2)) + 1; // sum <= 20
      } else {
        // Advanced
        num1 = Math.floor(Math.random() * 65) + 10; // 10 to 74
        const maxNum2 = 95 - num1;
        num2 = Math.floor(Math.random() * Math.min(25, maxNum2)) + 5; // sum <= 100
      }
  ...
  const handlePrint = async () => {
    audioManager.playPop();
    const paperElement = document.getElementById('math-addition-worksheet-paper');
    setIsExporting(true);
    await exportOrPrintElement({
      element: paperElement,
      filename: `Storybook_Addition_Worksheet_${wsTheme}`,
      title: `Addition Sums Worksheet - ${wsTheme.toUpperCase()}`,
    ...
    <div className="w-full max-w-4xl mx-auto flex flex-col items-center select-none text-black" id="math-addition-game">
      ...
      <div className={`w-full flex-col gap-8 text-black ${activeTab === 'worksheet' ? 'flex' : 'hidden print:flex'}`}>
            ...
            <div className="w-full flex justify-center print:w-full print:m-0 print:p-0">
              ...
              `}} />
              <div
                id="math-addition-worksheet-paper"

...
export default function App() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('wonderkids_playground_score');
    if (saved) {
      try {
  ...
  const handleGameWin = (awardedStars: number) => {
    setScoreState((prev) => {
      ...
      localStorage.setItem('wonderkids_playground_score', JSON.stringify(nextState));
      return nextState;
    });
  ...
  return (
    ...
    >
      ...
      <footer className="w-full text-center py-6 text-[10px] sm:text-xs font-bold text-amber-800/40 font-sans mt-auto">
        <span>© 2026 Children's Activity Playground. Built for active play & early learning.</span>
      </footer>
    </div>
  );
}
